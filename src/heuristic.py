    ...
    for row in range(puzzle.size):
        for column in range(puzzle.size):
            ...
            if column == expected_column:
                cost += compute_vertical_conflict(
                    puzzle, solution, digit, row, column, expected_column
                )

            # If current tile on its expected row.
            if row == expected_row:
                cost += compute_horizontal_conflict(
                    puzzle, solution, digit, row, column, expected_row
                )
...
def compute_horizontal_conflict(puzzle, solution, digit, row, column, expected_row):
    ...
    for index in range(puzzle.size):
        ...
        (
            ...
            compare_digit_expected_column,
        ) = solution.positions[compare_digit]
